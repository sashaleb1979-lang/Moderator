...
### Daily News на сервере

После запуска больше не нужно руками править news-конфиг в welcome-db.json.

1. Открой `/onboard panel` под модератором.
2. Нажми `Daily News`, затем `Настроить`.
3. В поле `Shadow compile` включи или выключи фоновую ежедневную сборку digest.
4. В поле `Автопубликация` включи `yes/да`, если хочешь, чтобы бот сам выпускал итоговый пост после часа публикации по МСК.
5. В `Public channel` и `Staff channel` можно вводить Channel ID, `#mention` или точное имя текстового канала на сервере.
6. В `Publish hour МСК` укажи час от `0` до `23`.
7. Сохрани модалку и сразу вернись в панель: там будет видно `manual-only` или `auto-publish` и текущие привязки каналов.
8. Если автопубликация выключена, Daily News продолжит собирать digest в shadow-режиме, а выпуск делается кнопкой `Опубликовать` или `Staff-only smoke`.
9. Если автопубликация включена, release tick идёт по текущему cadence `news.schedule.tickMinutes`, после часа публикации компилирует digest и публикует его один раз на текущий московский день с duplicate guard.
10. Если публичный выпуск упал после успешной сборки, следующий release tick возьмёт уже сохранённый digest и повторит именно публикацию, а не будет бессмысленно пересобирать день заново.
