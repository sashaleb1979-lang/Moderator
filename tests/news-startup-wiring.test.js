...
test("welcome-bot wires daily-news capture owners into live Discord events", () => {
  ...
  assert.match(source, /const \{ runDailyNewsReleaseTick \} = require\("\.\/src\/news\/scheduler"\);/);
  ...
  assert.match(source, /buildClientReadyPeriodicJobs\(\{[\s\S]*runDailyNewsReleaseTick: async \(\) => \{[\s\S]*const nowValue = nowIso\(\);[\s\S]*resolveDailyNewsPendingMemberRemovalResolutions\(client, nowValue\)[\s\S]*runSerializedDbTask\(\(\) => runDailyNewsReleaseTick\(\{[\s\S]*db,[\s\S]*now: nowValue,[\s\S]*saveDb,[\s\S]*client,[\s\S]*beforeCompile: \(\) => reconcileMemberRemovalEvents\([\s\S]*\),[\s\S]*\}\), "daily-news-release-tick"\)[\s\S]*\},[\s\S]*news: ensureNewsState\(db\)\.config,[\s\S]*\}\);/);
...
test("welcome-bot wires Daily News operator panel into moderator panel routing", () => {
  ...
  assert.match(source, /if \(await runInteractionHandlerSafely\([\s\S]*"Daily News panel modal interaction failed",[\s\S]*\(\) => handleDailyNewsPanelModalSubmitInteraction\(\{[\s\S]*interaction,[\s\S]*db,[\s\S]*isModerator,[\s\S]*parseRequestedChannelId,[\s\S]*resolveRequestedChannelId: \(value, fallbackChannelId = ""\) => resolveRequestedChannelIdFromGuild\(client, value, fallbackChannelId\),[\s\S]*saveDb,[\s\S]*now: nowIso\(\),[\s\S]*\}\)[\s\S]*\)\) \{/);
});