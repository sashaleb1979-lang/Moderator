...
- integration ownership cleanup hardened further around stale compat-shadow carry-over: dormant ELO/Tierlist sync writers теперь строят snapshot patch не из `db.config.integrations.*`, а из resolver-backed current integration record, поэтому manual/native fields вроде `mode` и `roleGrantEnabled` больше не могут быть случайно перезаписаны старым legacy shadow during import/apply. Startup import path в `src/db/store.js` тоже больше не берёт dormant `sourcePath` напрямую из `db.config.integrations.*`; он предпочитает persisted `db.sot.integrations.*.sourcePath`, так что SoT-owned source-path override сохраняется даже если compat shadow отстаёт
- integration channel read cutover now reaches the shared channel resolver too: `src/sot/resolver/channels.js` больше не читает `eloSubmit` / `eloGraphic` / `tierlistDashboard` / `tierlistSummary` channelId напрямую из compat shadow. Эти channel slots теперь строятся через resolver-backed integration snapshots, так что diagnostics и any hot path using `getResolvedChannelId(slot)` видят persisted SoT integration panel channel overrides даже если `db.config.integrations.*` отстаёт
- shared diagnostics snapshot coverage now includes integration panel metadata too: `diagnoseSotState()` больше не ограничивается `status` / `mode` / `sourcePath` для integration entries. Для `elo` и `tierlist` он теперь также отдаёт normalized `submitPanel` / `graphicBoard` / `dashboard` / `summary` snapshots и summary counter `trackedIntegrationPanels`, так что общая SoT diagnostics surface не теряет integration-owned panel state после read cutover
...
- текущий validation baseline после integration stale-shadow hardening slice: `node --check src/db/store.js`, `node --check src/integrations/elo-dormant.js`, `node --check src/integrations/tierlist-dormant.js`, `node --test tests/db-store.test.js tests/elo-dormant.test.js tests/tierlist-dormant.test.js tests/sot-integrations.test.js`, и `npm test` зелёные, 253/253
- текущий validation baseline после integration channel resolver cutover: `node --check src/sot/resolver/channels.js`, `node --test tests/sot-channels.test.js tests/sot-diagnostics.test.js`, и `npm test` зелёные, 254/254
- текущий validation baseline после diagnostics integration snapshot expansion: `node --check src/sot/diagnostics.js`, `node --test tests/sot-diagnostics.test.js tests/sot-report-integrations.test.js`, и `npm test` зелёные, 254/254
...
> Phase 5 partial status: `/sot-report` уже получил verify/recover/link-channel/manual-character-override/manual-role-override (base + tier-role domains)/manual-panel-override (`welcome` / `nonGgs` / `eloSubmit` / `eloGraphic` / `tierlistDashboard` / `tierlistSummary`)/orphan-cleanup actions, а alert pipeline живёт через throttled startup/moderator-sync/periodic log alerts по character health и multi-domain SoT drift. Integration ownership/read cluster тоже фактически дочищен: dormant sync, startup source-path resolution, shared channel resolver и shared diagnostics уже читают persisted SoT-first integration state без прямых runtime reads из compat shadow. Из реального хвоста теперь остаются wider behavior-level verification, operator/runbook burn-in и финальный completion gate, а не ещё один явный integration owner seam.
